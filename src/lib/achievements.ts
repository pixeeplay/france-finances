...
const CATEGORY_BADGES: Achievement[] = [
  ...
  { id: "badge_securite", icon: "\u2696\uFE0F", title: "Expert Sécurité", description: `Jouer ${BADGE_THRESHOLD} sessions Sécurité.`, category: "category", check: (s) => deckSessions(s, "securite") >= BADGE_THRESHOLD, progress: (s) => Math.min(100, Math.round((deckSessions(s, "securite") / BADGE_THRESHOLD) * 100)) },
  { id: "badge_etat", icon: "\uD83C\uDFDB\uFE0F", title: "Expert État", description: `Jouer ${BADGE_THRESHOLD} sessions Fonctionnement de l'État.`, category: "category", check: (s) => deckSessions(s, "etat") >= BADGE_THRESHOLD, progress: (s) => Math.min(100, Math.round((deckSessions(s, "etat") / BADGE_THRESHOLD) * 100)) },
